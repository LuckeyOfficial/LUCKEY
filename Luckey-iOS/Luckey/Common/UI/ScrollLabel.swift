...
public class AutoScrollLabel: UIView {
    ...
    private static let defaultFadeLength: CGFloat = 7.0
    private static let defaultLabelBufferSpace: CGFloat = 20  // Pixel buffer space between scrolling label
    private static let defaultPixelsPerSecond: Double = 30.0
    private static let defaultPauseTime: Double = 2

    public var scrollDirection = ScrollDirection.right {
        didSet {
            scrollLabelIfNeeded()
        }
    }
    ...
    public var scrollSpeed = AutoScrollLabel.defaultPixelsPerSecond {
        didSet {
            scrollLabelIfNeeded()
        }
    }
    ...
    // Defaults to 1.5
    public var pauseInterval = AutoScrollLabel.defaultPauseTime

    // Pixels, defaults to 20
    public var labelSpacing = AutoScrollLabel.defaultLabelBufferSpace
    ...
    // Defaults to 7
    public var fadeLength = AutoScrollLabel.defaultFadeLength {
        didSet {
...
//    }
    ...
    private var labels: [UILabel] = {
        ...
        for index in 0 ..< AutoScrollLabel.labelCount {
            ls.append(UILabel())
        }
        return ls
    }()
    ...
    private func commonInit() {
        ...
        self.scrollSpeed = AutoScrollLabel.defaultPixelsPerSecond
        self.pauseInterval = AutoScrollLabel.defaultPauseTime
        self.labelSpacing = AutoScrollLabel.defaultLabelBufferSpace
        self.fadeLength = AutoScrollLabel.defaultFadeLength
    ...
    public func observeApplicationNotifications() {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(AutoScrollLabel.scrollLabelIfNeeded),
    ...
    func scrollLabelIfNeededAction() {
        ...
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(AutoScrollLabel.enableShadow), object: nil)

        self.scrollView.layer.removeAllAnimations()

        let doScrollLeft = self.scrollDirection == ScrollDirection.left
        self.scrollView.contentOffset = doScrollLeft ? .zero : CGPoint(x: labelWidth + self.labelSpacing, y: 0)

        self.perform(#selector(AutoScrollLabel.enableShadow), with: nil, afterDelay: self.pauseInterval)
